...
while(True):
    ...
    if green_blobs:
        green_blob = find_max_red_blobs(green_blobs,img)
        print('找到了绿色色块')
        print("red_blobs,X:%s,Y:%s"%(green_blob.cx(),green_blob.cy()))
    ...
    print("调节已结束")           
    if data and green_blobs and red_blobs:
        ...
        if data_decoded[0] == frame_head and data_decoded[2] == frame_tail:#帧头帧尾 为 #a:
            ...
            if task_flag == 'a':
                if first_recieve_flag ==1:#区分第一次和后面的
                    ...
                    if green_blobs is not None and red_blobs is not None:
                        #send_data = '#0'+'X'+str(rect_points[rect_point_num][0])+'Y'+str(rect_points[rect_point_num][1])+'x'+str(red_blob.cx())+'y'+str(red_blob.cy())+';'
                        send_data = '#0'+'X'+str(red_blob.cx())+'Y'+str(red_blob.cx())+'x'+str(green_blob.cx())+'y'+str(green_blob.cy())+';'
                        print(send_data)
                        uart.write(send_data)
    else:
        print("miss data串口红绿色块")                    
